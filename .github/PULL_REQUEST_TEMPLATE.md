...
<!-- Concrete commands and their real output. Numbers, not adjectives.
  ...
  pnpm check                              exits 0
  pnpm --filter @rcrsr/rill-agent test    <N> passed
...
- `pnpm check` passes locally. CI repeats it across the Node matrix, plus knip,
  formatting, the custom lint rules, and the standards checker.
...
- New public API is exported from the package's `src/index.ts`. Sibling
  packages and consumers cannot reach deep paths.
- Version bumps follow the independent-versioning rule in CLAUDE.md. Maintainers
  handle releases.
-->

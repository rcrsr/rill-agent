...
- **foundry declaration build:** `@azure/msal-common` 16.13+ types `JoseHeader.jwk` as the WebCrypto global `JsonWebKey`, which broke the dts-bundle-generator check pass. The foundry `tsconfig.dts.json` adds `DOM` to `lib` for the declaration pass only; the emitted `dist/index.d.ts` is unchanged.

...
- **pnpm 12:** `packageManager` pinned to `pnpm@12.3.4`, `engines.pnpm` raised to `>=12`. pnpm 12 records the pin in the lockfile and rejects unrecognized `pnpm-workspace.yaml` keys on install. ([#75](https://github.com/rcrsr/rill-agent/pull/75))
- **Dependency sweep:** `@rcrsr/rill-dev` 0.2.5, vitest 5, `@types/node` 26.5, knip 6.35, lefthook 2.1.12, oxfmt 0.67, oxlint 1.82, tsup 8.5.1, hono 4.13.7, `@azure/identity` 4.13.2, OpenTelemetry SDK 0.222 / resources 2.11. ([#75](https://github.com/rcrsr/rill-agent/pull/75))
- **GitHub Actions:** `github/codeql-action` 4.37.6 -> 4.37.9. Every other pinned action is already at its latest release. ([#75](https://github.com/rcrsr/rill-agent/pull/75))
- **Standards (rill-dev 0.2.5):** dependabot version-update PRs disabled (`open-pull-requests-limit: 0`, STD-SUP-8); the pre-commit format hook excludes `pnpm-lock.yaml` so lockfile-only commits no longer halt the piped chain (STD-HOOK-5). ([#75](https://github.com/rcrsr/rill-agent/pull/75))
- **Dependency baseline:** Refreshed dependencies and shared tooling. ([#63](https://github.com/rcrsr/rill-agent/pull/63))

### Fixed

- **foundry declaration build:** `@azure/msal-common` 16.13+ types `JoseHeader.jwk` as the WebCrypto global `JsonWebKey`, which broke the dts-bundle-generator check pass. The foundry `tsconfig.dts.json` adds `DOM` to `lib` for the declaration pass only; the emitted `dist/index.d.ts` is unchanged. ([#75](https://github.com/rcrsr/rill-agent/pull/75))
